...
/// A `ProducerCfg` that points at an unreachable local port.
/// The `check_ready` probe in the route handler returns false within 500 ms,
/// short-circuiting any DB writes and returning 503 immediately.
...
///
/// The producer is configured with `127.0.0.1:19999` (nothing listening).
/// The route's `check_ready` probe fails within ~500 ms and the handler returns
/// 503 before any DB rows are written.
#[tokio::test]
...
///
/// Same unreachable-broker setup as AC5; the `check_ready` probe fires before
/// any DB inserts, so after the 503 response we confirm zero rows exist.
#[tokio::test]
async fn ac6_trigger_rolls_back_db_on_kafka_failure() {
    ...
    // Must not be a 2xx — broker unavailable means ingestion was not queued.
    assert!(
    ...
    // No ingestion_runs row must survive — DB writes happen after the probe
    // passes, so with an unreachable broker nothing is committed.
